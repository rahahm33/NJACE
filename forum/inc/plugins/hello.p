...
$(\'#resourcelink\').keyup(function() {
								...
								if (validURL($(this).val()) || (!($(this).val())) ) {	
									if (validURL($(this).val()) && (($(this).val())) ) {	
										$(\'#linkcheck\').html(\'<i class="icon-ok"></i>\');
									...
									$(\'#linkcheck\').html(\'<i class="icon-remove"></i>\');
...
function indexredir()
{
	//header('Location:'.defaults('url').'resources/myresources.php');
}
...
function threadinfid()
{
	...
	if (!$mybb->input['mod'] == 1)
	{
		...
		else
		{
			//echo $mybb->input['fid'];
			//exit;
			header('Location:'.defaults('url').'resources/myresources.php?fid='.$mybb->input['fid']);
		}
	}
}
...
function main_menu()
{
	...
	$mybb->user['headinclude'] = getTemplate( $_SERVER['DOCUMENT_ROOT'] . '/views/headinclude.html');
	$mybb->user['mainmenu'] = getTemplate( $_SERVER['DOCUMENT_ROOT'] . '/views/header.html');
...
function profile_sites()
{
	...
	foreach ($groups as $gid)
	{
		if (!empty($gid))
		{
			$memprofile['rsites'] .= '<tr><td class="trow1"><a style="font-weight:bold" href="/sites/?id='.$gid.'">'.sites_info($gid,'short_name') . '</a></td><td class="trow1"><span class="float_left">'. users_role($memprofile['uid'],$gid) .'</span></td></tr>';
		}
	}
